...
GRID_TITLE_FONTSIZE = 10
GRID_AXIS_LABELSIZE = 9
GRID_TICK_LABELSIZE = 7
GRID_CELL_TEXT_FONTSIZE = 4

COLORBAR_LABELSIZE = 9
COLORBAR_TICK_LABELSIZE = 7

GENERAL_TITLE_FONTSIZE = 10
GENERAL_AXIS_LABELSIZE = 9
GENERAL_TICK_LABELSIZE = 8
GENERAL_LEGEND_FONTSIZE = 8
...
def plot_grid(
    ...
    *,
    ...
    for j, cell in enumerate(grid.parking_cells):
        x, y = _cell_xy(cell)
        ax.scatter(x, y, marker="s", s=55, edgecolor="black", linewidth=0.6)
        if show_indices:
            ax.text(
                x,
                y,
                ...
                ha="center",
                va="center",
                fontsize=GRID_CELL_TEXT_FONTSIZE,
            )
...
def plot_error_map(
    ...
    *,
    ...
    im = ax.imshow(err_map, origin="upper", vmin=0, vmax=vmax)
...
def plot_simulation_overview(
    ...
    *,
    ...
    plot_grid(result.grid, ax=axes[0], title="Parking cells")
    plot_destination_density(result.grid, result.demand, ax=axes[1], title="Destination density")
    plot_theta_map(result.grid, result.theta, ax=axes[2], title="True theta")
